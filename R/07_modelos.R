...
(
    sampleData = data.frame(
        condition = factor(
            rep(c("ctrl_minus", "ctrl_plus", "ko_minus", "ko_plus"), 3)
        ),
...
rse_gene_SRP045638$sra_attribute.disease <- factor(
    tolower(rse_gene_SRP045638$sra_attribute.disease)
)
...
rse_gene_SRP045638$prenatal <- factor(
    ifelse(rse_gene_SRP045638$sra_attribute.age < 0, "prenatal", "postnatal")
)
...
df <- as.data.frame(
    colData(rse_gene_SRP045638)[,
        c("prenatal", "sra_attribute.RIN", "sra_attribute.sex")
    ]
)
